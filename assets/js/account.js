...
window.LUNA.onReady(function () {
  ...
  function renderAuth(message) {
    ...
    <div class="acc-card">
      ...
      <p class="acc-admin">Placed an order as guest? <a href="track.html">Track your order →</a></p>
      <p class="acc-admin" style="margin-top:.5rem">Store staff? <a href="admin.html">Log in as admin →</a></p>
    </div>`;
  ...
  async function renderAccount(user) {
    ...
    // Store staff see a shortcut into the admin panel.
    sb.rpc('is_admin').then(({ data }) => {
      if (data) root.querySelector('[data-admin-slot]').innerHTML = `<a class="btn" href="admin.html">Admin Panel</a>`;
    });
